...
public class CameraFollow : MonoBehaviour {
	...
	void Start(){
        ...
        gameSceneManager = GameObject.Find("Main Camera").GetComponent<GameScene>();
	}
		...
		if(target != null){
			if(target.position.z >= 0){
				if(shake == false){
                    ...
                    posCamera.z = Mathf.Lerp(posCamera.z, target.position.z + distance, gameSceneManager.currentSpeed); //* Time.deltaTime);
